...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    Page<Event> getEventsWithStates(Collection<Long> initiator_id, Collection<String> state,
                                    Collection<Long> category_id, LocalDateTime eventDate, LocalDateTime eventDate2,
                                    Pageable pageable);

    @Query("select e from Event e " +
            "where e.initiator.id in ?1 and e.category.id in ?2 and e.eventDate > ?3 and e.eventDate < ?4")
    Page<Event> getEventsWithoutStates(Collection<Long> initiator_id, Collection<Long> category_id,
